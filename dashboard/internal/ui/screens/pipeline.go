...
func (m PipelineModel) handleKey(msg tea.KeyMsg) (PipelineModel, tea.Cmd) {
	...
	case "d":
		if app, ok := m.CurrentApp(); ok {
			...
			if len(candidates) == 0 {
				m.flash = "No CV PDF found for this application — generate one with /dafe-career-os pdf"
			} else {
	...
	case "D":
		if app, ok := m.CurrentApp(); ok {
			...
			if !found || entry.HTMLPath == "" {
				m.flash = "No source HTML found for this application — run /dafe-career-os pdf first"
				return m, nil
			}
...
func (m PipelineModel) renderHelp() string {
	...
	brand := lipgloss.NewStyle().Foreground(m.theme.Overlay).Render("DAFE Career OS")
