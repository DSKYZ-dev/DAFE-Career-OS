...
- Any directory outside the dafe-career-os project — for example, parent-directory repos containing the user's product code, sibling project directories, or other unrelated codebases on the same machine
...
- `{"status": "update-available", "local": "1.0.0", "remote": "1.1.0", "changelog": "..."}` → tell the user:
  > "dafe-career-os update available (v{local} → v{remote}). Your data (CV, profile, tracker, reports) will NOT be touched. Want me to update?"
...
The user can also say "check for updates" or "update dafe-career-os" at any time to force a check.
...
- **Interactive Codex:** run `codex` in the repo root. Slash commands are not guaranteed in Codex, so ask Codex to run the requested mode directly if `/dafe-career-os` is unavailable.
- **Headless Codex:** use `codex exec "prompt"` for one-shot workers.
- **Examples:** `Run dafe-career-os scan mode`, `Run dafe-career-os pipeline mode for data/pipeline.md`, `Run dafe-career-os pdf mode`, `Run dafe-career-os tracker mode`, `Evaluate this JD with dafe-career-os auto-pipeline: https://company.com/jobs/123`
...
> "dafe-career-os works fully on Antigravity CLI's free tier — no API key or paid subscription needed. See [FREE_TIER.md](docs/FREE_TIER.md) for setup (`agy auth login`, daily limits, and batch tips)."
...
> - Run the scan entrypoint for your CLI to search portals: `/dafe-career-os scan`, `/dafe-career-os-scan`, or ask Codex to run `scan`
> - Open the command menu for your CLI: `/dafe-career-os`, the CLI-specific alias, or ask Codex to show the available dafe-career-os modes
>
...
If the user accepts, use the `/loop` or `/schedule` skill (if available) to set up a recurring scan entrypoint for their CLI (`/dafe-career-os scan`, `/dafe-career-os-scan`, or the equivalent Codex prompt). If those aren't available, suggest adding a cron job or remind them to run the scan mode periodically.
