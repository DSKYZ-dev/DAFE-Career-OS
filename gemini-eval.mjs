#!/usr/bin/env node
/**
 * gemini-eval.mjs — Gemini-powered Job Offer Evaluator for dafe-career-os
 *
...
const PATHS = {
  ...
  evaluate:    join(ROOT, '.claude', 'skills', 'dafe-career-os', 'SKILL.md'),
  cv:          join(ROOT, 'cv.md'),
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║           dafe-career-os — Gemini Evaluator (free-tier)             ║
╚══════════════════════════════════════════════════════════════════╝
...
function validateEvaluationShape(text) {
  ...
  if (issues.length > 0) {
    throw new Error(`Gemini returned an invalid dafe-career-os report: ${issues.join('; ')}`);
  }
}
...
// ---------------------------------------------------------------------------
const systemPrompt = `You are dafe-career-os, an AI-powered job search assistant.
...
// ---------------------------------------------------------------------------
console.log('\n' + '═'.repeat(66));
console.log('  DAFE CAREER OS EVALUATION — powered by Google Gemini');
