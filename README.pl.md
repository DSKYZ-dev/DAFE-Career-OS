...
DAFE Career OS ([dafe-career-os.org](https://dafe-career-os.org), znany też jako **careerops**) zamienia dowolne AI CLI w pełne centrum dowodzenia poszukiwaniem pracy. Zamiast ręcznego śledzenia aplikacji w arkuszu kalkulacyjnym, dostajesz pipeline zasilany AI, który:
...
> **Ważne: to NIE jest narzędzie do masowego wysyłania aplikacji.** DAFE Career OS to filtr — pomaga znaleźć te kilka ofert wartych twojego czasu spośród setek. System stanowczo odradza aplikowanie na oferty z oceną poniżej 4.0/5. Twój czas jest cenny, podobnie jak czas rekrutera. Zawsze sprawdzaj przed wysłaniem.

DAFE Career OS działa agentowo: Claude Code nawiguje po stronach kariery z Playwright, ocenia dopasowanie rozumując nad twoim CV kontra opis stanowiska (nie przez dopasowanie słów kluczowych) i dostosowuje CV do każdego ogłoszenia.
...
To sklonuje najnowszą wersję do `./dafe-career-os` i zainstaluje zależności. Następnie:
...
**Przy pierwszym uruchomieniu dafe-career-os przeprowadza Cię przez konfigurację — CV, profil i docelowe stanowiska — wyłącznie przez rozmowę. Nic nie trzeba edytować ręcznie.**
...
DAFE Career OS to jedna komenda slash z wieloma trybami:

```text
/dafe-career-os                    → Pokaż wszystkie dostępne komendy
/dafe-career-os {wklej ofertę}     → Pełny auto-pipeline (ocena + PDF + tracker)
/dafe-career-os scan               → Skanuj portale w poszukiwaniu nowych ofert
/dafe-career-os pdf                → Generuj CV zoptymalizowane pod ATS
/dafe-career-os batch              → Wsadowa ocena wielu ofert
/dafe-career-os tracker            → Podgląd statusu aplikacji
/dafe-career-os apply              → Wypełnianie formularzy aplikacyjnych z AI
/dafe-career-os pipeline           → Przetwarzanie kolejki URL
/dafe-career-os contacto           → Wiadomość na LinkedIn
/dafe-career-os deep               → Szczegółowe badanie firmy
/dafe-career-os training           → Ocena kursu/certyfikatu
/dafe-career-os project            → Ocena projektu portfolio
```

Możesz też po prostu wkleić URL oferty lub jej treść — dafe-career-os automatycznie to wykryje i uruchomi pełny pipeline.
...
DAFE Career OS obsługuje główne polskie portale IT. Dwa z nich — JustJoin.it i NoFluffJobs — mają publiczne API i mogą być zintegrowane jako źródła Level 0 (zero tokenów, brak WebSearch, świeże dane w czasie skanowania). Pozostałe portale wymagają weryfikacji ręcznej lub przez Playwright.
...
DAFE Career OS uwzględnia specyfikę polskiego rynku pracy przy ocenianiu ofert:
...
**dafe-career-os to lokalne narzędzie open source, NIE usługa hostingowa.** Korzystając z tego oprogramowania, potwierdzasz:
...
Kod jest licencjonowany na [MIT](LICENSE). Nazwa i marka „dafe-career-os" są regulowane przez [Politykę Znaków Towarowych](TRADEMARK.md) — dozwolone dla użytku społecznościowego, zastrzeżone dla komercyjnego nazewnictwa produktów i endorsementu.
