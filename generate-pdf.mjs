 ...
 *
 * Usage:
 *   node dafe-career-os/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--report=NNN]
 *
 ...
 * dafe-career-os root with forward slashes. One row per PDF path; when a report
...
export async function renderHtmlToPdf(html, outputPath, opts = {}) {
  ...
  const tmpHtmlPath = resolve(baseDir, `.dafe-career-os-render-${randomUUID()}.html`);
  ...
  try {
    ...
    // Update manifest if reportNum provided (library callers can opt-in)
    if (reportNum) {
      try {
        updatePDFManifest(reportNum, outputPath, opts.inputPath || '', format);
        console.log(`🔗 Manifest: data/pdf-index.tsv updated (report ${reportNum})`);
      } catch (err) {
