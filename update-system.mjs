...
/**
 * update-system.mjs — Safe auto-updater for dafe-career-os
 *
...
const SYSTEM_PATHS = [
  ...
  'autonomous-loop.mjs',
  'need-a-job-now.mjs',
  'career-orchestrator.mjs',
  'continuous-worker.mjs',
  'run-pipeline-bg.mjs',
  'dafe-career-os-launcher.mjs',
  'dashboard.mjs',
  'cloud-eval.mjs',
  'llm-helper.mjs',
  'model-registry.mjs',
...
async function check() {
  ...
  const [rawVersion, releaseRaw] = await Promise.all([
    ...
    curlGet(RELEASES_API, [
      ...
      '--header', 'User-Agent: dafe-career-os-update-checker',
    ]),
  ]);
