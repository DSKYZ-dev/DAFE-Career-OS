...
> Claude Code marketplace metadata). These plugins extend dafe-career-os itself.
...
A plugin is a directory under `plugins/` (bundled, shipped with dafe-career-os) or
...
dafe-career-os is plain ESM with no build step, so the engine **cannot truly
