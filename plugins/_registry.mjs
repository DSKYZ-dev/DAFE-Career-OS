...
/** Find a registry entry by bare id, full name, or `dafe-career-os-plugin-<id>`. */
export function findInRegistry(root, nameOrId) {
  const reg = loadRegistry(root);
  return reg.plugins.find(p =>
    p.id === nameOrId || p.name === nameOrId || p.name === `dafe-career-os-plugin-${nameOrId}`) || null;
}
...
export function validateRegistryEntry(e, { idRe, hookKinds, reservedEnv }) {
  const errs = [];
  if (typeof e.name !== 'string' || !e.name.startsWith('dafe-career-os-plugin-')) errs.push('name must start with "dafe-career-os-plugin-"');
  if (typeof e.id !== 'string' || !idRe.test(e.id)) errs.push('invalid id');
  if (e.name && e.id && e.name !== `dafe-career-os-plugin-${e.id}`) errs.push('name must equal dafe-career-os-plugin-<id>');
