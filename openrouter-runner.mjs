#!/usr/bin/env node
/**
 * dafe-career-os OpenRouter Runner
...
async function fetchJobPage(url) {
  ...
  try {
    const r = await fetch(url, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; dafe-career-os/1.0)' }
    });
...
// search-query companies are handled by the full /dafe-career-os scan pipeline.
