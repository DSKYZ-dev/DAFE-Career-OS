 ...
 *
 * Run: node dafe-career-os/merge-tracker.mjs [--dry-run] [--verify]
 */
 ...
 * and use the dafe-career-os lock-name prefix. Invalid values are ignored and the
...
function resolveTrackerLockDir(envValue, lockKey) {
  ...
  const fallback = join(tmpRoot, `dafe-career-os-merge-tracker-${lockKey}.lock`);
  ...
  if (!basename(candidate).startsWith('dafe-career-os-merge-tracker-')) return fallback;
  return candidate;
}
