...
// in modes/pipeline.md and expected by /dafe-career-os pipeline.
const PIPELINE_SKELETON = `# Pipeline — Pending URLs

Paste job URLs below as \`- [ ] {url}\` then run \`/dafe-career-os pipeline\`.
...
async function main() {
  ...
  const remoteOnly = config.location?.remote_only === true;
  ...
  const tasks = targets.map(company => async () => {
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        // Remote-only filter: when enabled, require location to be remote-equivalent
        if (remoteOnly) {
          const loc = typeof job.location === 'string' ? job.location.toLowerCase() : '';
          const isRemote = loc.includes('remote') || loc.includes('worldwide') || loc.includes('anywhere') || loc.includes('global') ||
            loc.includes('united states') || loc.includes('usa') || loc.includes(' us ') || loc.startsWith('us ') || loc.endsWith(' us') || loc === 'us';
          if (!isRemote) {
            totalFilteredLocation++;
            continue;
          }
  ...
  console.log(`\n→ Run /dafe-career-os pipeline to evaluate new offers.`);
