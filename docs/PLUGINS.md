...
Plugins extend dafe-career-os with integrations that need an API key or talk to an
...
If a plugin's files change without a version bump, dafe-career-os **blocks it** and
...
1. Develop locally, then publish your plugin as its **own public GitHub repo**
   named exactly `dafe-career-os-plugin-<name>` (the template repo gives you the
