...
- **Listed** — one entry in `plugins-registry.json`; the code stays in the
  author's `dafe-career-os-plugin-<name>` repo; users `add` it. The default, low
  burden.
...
- **Naming + identity:** repo is `dafe-career-os-plugin-<name>`, `id` == name minus
...
- **Wording (public-forever):** description / README / skill carry no commercial
  / hosted-service / monetization language. dafe-career-os is free and local-first;
...
**not** registry-listed. It can still be a `dafe-career-os-plugin-<name>` repo users
