...
/**
 * test-all.mjs — Comprehensive test suite for dafe-career-os
 *
 ...
 * any freshly cloned dafe-career-os checkout with only Node.js available.
 *
 ...
 *
 * @param {string} path - Path relative to the dafe-career-os repository root.
...
const scripts = [
  ...
  // tests inside their active dafe-career-os workspace.
...
const systemFiles = [
  ...
  '.claude/skills/dafe-career-os/SKILL.md',
  '.opencode/skills/dafe-career-os/SKILL.md',
  '.qwen/skills/dafe-career-os/SKILL.md',
  '.antigravitycli/skills/dafe-career-os/SKILL.md',
  '.grok/skills/dafe-career-os/SKILL.md',
];
...
for (const skillPath of ['.claude/skills/dafe-career-os/SKILL.md', '.agents/skills/dafe-career-os/SKILL.md']) {
  if (!fileExists(skillPath)) {
    fail(`${skillPath} is missing`);
    continue;
  }
  const skill = readFile(skillPath);
  if (skill.includes('/dafe-career-os latex')) {
    pass(`${skillPath} exposes /dafe-career-os latex in discovery menu`);
  } else {
    fail(`${skillPath} does not expose /dafe-career-os latex in discovery menu`);
  }
}
...
try {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'dafe-career-os-missing-pipeline-'));
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-portals-validator-'));
...
const canonicalSkill = '.agents/skills/dafe-career-os/SKILL.md';
const symlinks = [
  '.claude/skills/dafe-career-os/SKILL.md',
  '.opencode/skills/dafe-career-os/SKILL.md',
  '.qwen/skills/dafe-career-os/SKILL.md',
...
if (
  ...
  /\/dafe-career-os/.test(canonicalContent ?? '')
) {
  pass('dafe-career-os skill router documents the Codex invocation model');
} else {
  fail('dafe-career-os skill router is missing Codex invocation guidance');
}
...
if (
  ...
  /(slash commands?.*not guaranteed|prompt|\/dafe-career-os.*unavailable)/i.test(agentsDoc)
) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'dafe-career-os-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'dafe-career-os');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'dafe-career-os');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'dafe-career-os');
    mkdirSync(canonicalDir, { recursive: true });
    mkdirSync(claudeDir, { recursive: true });
    mkdirSync(opencodeDir, { recursive: true });

    const fixtureSkill = '---\nname: dafe-career-os\n---\n\n# canonical skill\n';
    const pointer = '../../../.agents/skills/dafe-career-os/SKILL.md';
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'dafe-career-os-ensure-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'dafe-career-os');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'dafe-career-os');
    mkdirSync(canonicalDir, { recursive: true });
    ...
    const grokSkill = readFileSync(join(fixtureRoot, '.grok', 'skills', 'dafe-career-os', 'SKILL.md'), 'utf-8');
    const claudeSkill = readFileSync(join(claudeDir, 'SKILL.md'), 'utf-8');
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'dafe-career-os-skills-unreadable-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'dafe-career-os');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'dafe-career-os');
    mkdirSync(canonicalDir, { recursive: true });
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'dafe-career-os-skills-entry-dir-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'dafe-career-os');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'dafe-career-os');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'dafe-career-os');
    ...
    const materialized = updater.materializeSkillEntrypoints(fixtureRoot);
    const opencodeSkill = readFileSync(join(opencodeDir, 'SKILL.md'), 'utf-8');
    if (JSON.stringify(materialized) === JSON.stringify(['.opencode/skills/dafe-career-os/SKILL.md']) && opencodeSkill === fixtureSkill) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'dafe-career-os-skill-git-'));
  ...
  try {
    ...
    writeFileSync(join(claudeDir, 'SKILL.md'), pointer);
    writeFileSync(join(opencodeDir, 'SKILL.md'), pointer);
    gitRun(['add', '--', '.agents/skills/dafe-career-os/SKILL.md']);

    const pointerBlob = gitRun(['hash-object', '-w', '--stdin'], { input: pointer });
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.claude/skills/dafe-career-os/SKILL.md`]);
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.opencode/skills/dafe-career-os/SKILL.md`]);
    ...
    const claudeIndex = gitRun(['ls-files', '-s', '--', '.claude/skills/dafe-career-os/SKILL.md']);
    const opencodeIndex = gitRun(['ls-files', '-s', '--', '.opencode/skills/dafe-career-os/SKILL.md']);
    ...
    const claudeBlob = gitRaw(['show', ':.claude/skills/dafe-career-os/SKILL.md']);
    const opencodeBlob = gitRaw(['show', ':.opencode/skills/dafe-career-os/SKILL.md']);
...
try {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'dafe-career-os-migrate-'));
  try {
  ...
  const followupTmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-followup-link-'));
  try {
...
try {
  ...
  const dedupTmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-dedup-'));
  try {
...
try {
  const rebuildTmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-rebuild-'));
  try {
...
try {
  const locTmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-dedup-loc-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-merge-'));
  try {
...
try {
  const col912Tmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-merge-912-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-merge-lock-'));
  /**
    ...
    const result = new Promise(resolve => {
      const child = spawn(NODE, ['merge-tracker.mjs'], {
        ...
        env: {
          ...
          CAREER_OPS_TRACKER_LOCK: join(mergeTmp, 'dafe-career-os-merge-tracker-fixture.lock'),
...
if (!sqliteAvailable) {
  ...
  try {
    const idxTmp = mkdtempSync(join(tmpdir(), 'dafe-career-os-index-'));
    try {
...
try {
  ...
  const hit = sj.detect({ name: 'SJ', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=dafe-career-os' });
  ...
  const fakeJobs = {
    jobs: [
      { title: 'Senior Dev', url: 'https://solid.jobs/o/abc123/dafe-career-os', company: 'Acme Corp', locations: ['Warszawa', 'Remote'] },
      { title: 'Junior Dev', url: 'https://solid.jobs/o/def456/dafe-career-os', company: 'Beta Inc', locations: ['Kraków'] },
    ],
  };
  const parsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=dafe-career-os' },
  ...
  if (parsed[0].title === 'Senior Dev' && parsed[0].url === 'https://solid.jobs/o/abc123/dafe-career-os') {
  ...
  const noCompanyJobs = { jobs: [{ title: 'Tester', url: 'https://solid.jobs/o/xyz/dafe-career-os', locations: [] }] };
  ...
  const mixedJobs = {
    jobs: [
      { title: 'Has URL', url: 'https://solid.jobs/o/1/dafe-career-os', company: 'A', locations: [] },
  ...
  const stringLocJobs = { jobs: [{ title: 'Dev', url: 'https://solid.jobs/o/2/dafe-career-os', company: 'X', locations: 'Warsaw' }] };
  ...
  const malformedMembers = { jobs: [null, 7, { title: 'OK', url: 'https://solid.jobs/o/3/dafe-career-os', company: 'Z' }] };
  const safeParsed = await sj.fetch(
    { name: 'SolidJobs IT', careers_url: 'https://solid.jobs/public-api/offers/it?campaign=dafe-career-os' },
    { transport: 'http', fetchJson: async () => malformedMembers, fetchText: async () => '' },
  );
  if (safeParsed.length === 1 && safeParsed[0].url === 'https://solid.jobs/o/3/dafe-career-os') {
...
try {
  ...
  // Release Please tags carry the component prefix (dafe-career-os-v1.9.0); the
  // prefix must be stripped or the releases-API fallback is dead code (#923).
  if (parse('dafe-career-os-v1.9.0') === '1.9.0') {
    pass('SEMVER_RE parses Release Please component-prefixed tag (dafe-career-os-v1.9.0 → 1.9.0)');
  } else {
    fail(`SEMVER_RE failed on dafe-career-os-v1.9.0 (got ${parse('dafe-career-os-v1.9.0')}) — releases-API fallback is dead code (#923)`);
  }
  ...
  if (parse('dafe-career-os') === null && parse('v1.9') === null) {
    pass('SEMVER_RE rejects non-semver input');
  } else {
    fail(`SEMVER_RE matched non-semver input (dafe-career-os → ${parse('dafe-career-os')}, v1.9 → ${parse('v1.9')})`);
  }
} catch (e) {
...
try {
  ...
  const goodEntry = { name: 'dafe-career-os-plugin-x', id: 'x', repo: 'https://github.com/a/dafe-career-os-plugin-x', author: 'a', hooks: ['ingest'], requiredEnv: [], allowedHosts: ['api.x.com'], license: 'MIT', version: '1.0.0', sha: 'a'.repeat(40) };
  ...
  if (reg.validateRegistryEntry({ ...goodEntry, name: 'evil-x' }, regOpts).length > 0) pass('registry: name must start with dafe-career-os-plugin-');
  ...
  if (install.parseRepoArg('alice/dafe-career-os-plugin-foo').id === 'foo') pass('install: owner/dafe-career-os-plugin-foo parses to id "foo"');
  ...
  if (nameRej) pass('install: refuses a repo not named dafe-career-os-plugin-*');
