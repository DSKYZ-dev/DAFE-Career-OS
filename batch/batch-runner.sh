...
# dafe-career-os batch runner — standalone orchestrator for claude -p workers
...
usage() {
  cat <<'USAGE'
dafe-career-os batch runner — process job offers in batch via claude -p workers
...
main() {
  ...
  echo "=== dafe-career-os batch runner ==="
  if (( LIMIT > 0 )); then
