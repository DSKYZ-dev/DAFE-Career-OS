...
> "No active applications to follow up on. Apply to some roles first with `/dafe-career-os` and come back when they're aging."
...
- Suggest the user run `/dafe-career-os contacto {company}` to find the right person first
...
> - Trying a different contact via `/dafe-career-os contacto`
...
**To:** {email or "No contact found — run `/dafe-career-os contacto` first"}
