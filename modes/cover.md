...
Works in two modes:
- **Slug mode:** `/dafe-career-os cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/dafe-career-os cover` or JD pasted directly — starts from scratch
...
When invoked as `/dafe-career-os cover {slug}`:
