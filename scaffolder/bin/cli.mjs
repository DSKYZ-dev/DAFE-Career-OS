#!/usr/bin/env node
// dafe-career-os scaffolder — one-command install.
...
const USAGE = `dafe-career-os — set up an AI job search workspace.
...
async function latestTag() {
  try {
    const res = await fetch(LATEST_RELEASE, {
      headers: { "User-Agent": "dafe-career-os-cli", Accept: "application/vnd.github+json" },
    });
...
async function main() {
  ...
  const target = dirArg || "dafe-career-os";
  ...
  // Pretty path for messages: "./dafe-career-os" for relative, as-is for absolute.
  ...
  console.log(`\n→ Cloning dafe-career-os${tag ? ` @ ${tag}` : ""} into ${display} ...`);
  ...
  console.log(`\n✓ dafe-career-os is ready in ${display}\n`);
  ...
  console.log("\ndafe-career-os is AI-agnostic — Claude Code, Codex, Qwen, OpenCode, Copilot, Antigravity and Grok all work.");
