...
This system is a fork of the original **career-ops** (MIT) by [santifer](https://santifer.io). The architectural patterns are derived from that project. This fork includes substantial independent development — see `ATTRIBUTIONS.md` for the full breakdown.
...
Instead of registering individual `.toml` files for every slash command, all subcommands are routed through the single unified skill defined in `.agents/skills/dafe-career-os/SKILL.md`.

You can invoke the command center or any of its modes directly within your CLI:

* `/dafe-career-os` (Shows the Command Center menu)
* `/dafe-career-os {JD text or URL}` (Runs the auto-evaluation pipeline)
* `/dafe-career-os [subcommand]` (Runs a specific subcommand)
...
> - Run `/dafe-career-os scan` to search portals
> - Run `/dafe-career-os` to see all commands
>
...
If the user accepts, use the `/loop` or `/schedule` skill (if available) to set up a recurring `/dafe-career-os scan`. If those aren't available, suggest adding a cron job or remind them to run `/dafe-career-os scan` periodically.
